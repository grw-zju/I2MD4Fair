...
def build_ablation_model(config, dataset, modality_dims, args, device):
    ...
    class AblationModel(nn.Module):
        def __init__(self, n_users, n_items, embed_dim, modality_dims,
                     ...
                     use_avg_fusion=False, n_protos=64):
            super().__init__()
            ...
            if use_intra:
                ...
                for k in modality_dims:
                    if use_soft_proto:
                        self.intra_mdm[k] = IntraMDM(n_items, n_protos, embed_dim, args.eps)
                    else:
                        self.intra_mdm[k] = IntraMDM(n_items, n_protos, embed_dim, args.eps)
                        self.intra_mdm[k].soft_clustering = SoftPrototypeClustering(n_items, n_protos, embed_dim, args.eps)
            ...
            if use_hgcn and not use_intra:
                ...
                for k in modality_dims:
                    self.prototypes_abl[k] = nn.Parameter(torch.randn(n_protos, embed_dim))
            ...
            self.info_nce = InfoNCELoss(tau=args.tau)
            self.p_norm = args.p_norm if use_adaptive else 1.0
            self.lam = args.lam
        ...
        def forward(self, graph_norm, modality_features, user_ids, pos_ids, neg_ids):
            ...
            final_id = torch.mean(torch.stack(embs_list, dim=1), dim=1)
            user_embs = final_id[:self.n_users]
            item_embs = final_id[self.n_users:]
            ...
            for k in modality_features:
                ...
                if self.use_intra:
                    Z_v_k, _ = self.intra_mdm[k](Z_v_k)
                Z_u_k = self.modality_user_embs[k].weight
                mod_all = torch.cat([Z_u_k, Z_v_k], dim=0)
                mod_embs = [mod_all]
                for _ in range(args.n_layers):
                    mod_all = torch.sparse.mm(graph_norm, mod_all) if graph_norm.is_sparse else graph_norm @ mod_all
                    mod_embs.append(mod_all)
                mod_final = torch.mean(torch.stack(mod_embs, dim=1), dim=1)
                Z_u_k = mod_final[:self.n_users]
                Z_v_k = mod_final[self.n_users:]
                Z_v_dict[k] = Z_v_k
                Z_u_dict[k] = Z_u_k
            ...
            for k in modality_features:
                ...
                pos_k = Z_v_dict[k][pos_ids]
                neg_k = Z_v_dict[k][neg_ids]
                ...
                if self.use_ib:
                    ...
                    Z_v_k_raw = self.modality_encoders[k](M_k)
                    mu, logvar = self.club_estimators[k](M_k)
                    log_pos = self.club_estimators[k].log_prob(Z_v_k_raw, mu, logvar).sum(dim=-1)
                    shuffled = Z_v_k_raw[torch.randperm(Z_v_k_raw.shape[0])]
            ...
            info_nce_loss = self.info_nce({k: Z_v_dict[k] for k in Z_v_dict}, batch_item_ids)

            l2_reg = sum(torch.norm(p, 2) ** 2 for p in self.parameters()) / 2

            total_loss = bpr_loss + args.lambda1 * adaptive_loss + args.lambda2 * info_nce_loss + args.lambda3 * l2_reg
    ...
    return AblationModel(n_users, n_items, embed_dim, modality_dims,
                        n_protos=args.n_protos, **cfg).to(device)
